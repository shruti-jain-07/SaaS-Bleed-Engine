...
from backend.app.models.models import ContractAnalysisModel  # <-- Updated here
...
class AnalyticsService:
    ...
    def get_dashboard_summary(db: Session) -> Dict[str, Any]:
        """Aggregates transaction and contract intelligence metrics for executive dashboard reporting."""
        repo = TransactionRepository(db)
        transactions = repo.get_all_transactions()

        total_spend = (
            sum(t.amount for t in transactions) if transactions else 0.0
        )
